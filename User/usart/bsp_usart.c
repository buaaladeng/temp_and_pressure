...
void USART2_Config(void)
{
	...
	/* WKUP GPIO config */
	/* Configure WKUP (PA.00) as alternate function push-pull */
	...
	/* USART2 mode config */
	USART_InitStructure.USART_BaudRate = 9600;    //433模块通信波特率默认9600
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
	USART_InitStructure.USART_Parity = USART_Parity_No ;
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
...
void USART3_Config(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	/* Configure Reset_4G(PC.03) as output push-pull */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	/* Configure SET_433 Module (PC.05) as output push-pull */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	/* Configure EN_433 Module (PB.00) as output push-pull */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	/* USART3 mode config */
	USART_InitStructure.USART_BaudRate = 9600;         //3G模块通信波特率默认115200
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
	USART_InitStructure.USART_Parity = USART_Parity_No ;
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
...
/**
  * @brief  USART3 RX DMA 配置，内存到外设(USART3->DR)
  * @param  无
  * @retval 无
  */
...
/**
  * @brief  USART2 RX DMA 配置，内存到外设(USART2->DR)
  * @param  无
  * @retval 无
  */
