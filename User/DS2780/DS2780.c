...
uint8_t DS2780_Test(void)
{
    ...
    uint8_t   FULL_status=0;
	  ...
	  if (PowerOffReset ==0)                             //没有发生掉电，则直接从Flash中读出电池容量系数
	  {
			DataRead_From_Flash(0,1,0, &RegB,1);             //从Flash中读出电池容量系数
			if(RegB>113)    //防止发生Flash异常复位
			{
        RegB=113;
				printf("\n\r RegB  ERROR!!\r\n");             //测试使用
      }
		...
		{	
			DataWrite_To_Flash(0,1,0, &RegB,1);              //初次上电时，将电池容量系数写入Flash
...
//    dats7 = get_FULL();      //读现有电量状态，百分比
	  printf("\n\r BATT Capacity left:%d mAh\r\n", dats6);   //测试使用
		...
		{
			set_ACR(1000);
			Delay_ms(1000);       // 延时10ms，具体延时时间有待确认
			if(RegB !=0)           //防止出现溢出
			{
        RegB--;	
    ...
    //FULL_status = FULL_status*2.55;
		...
		{
       DataWrite_To_Flash(0,1,0, &RegB,1);   //将电池容量系数写入Flash
    }
    return FULL_status;
}
/*******************************************************************************
...
unsigned char read_bit(void)
{
  unsigned char result;
	...
	result = GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_7); // 采样result 信号
