 ...
 * 调用  ：内部调用
 */
void Time_Auto_Regulate(struct DeviceSet* TimeCorrect)
{
//	  u32 Tmp_YY = 0xFF, Tmp_MM = 0xFF, Tmp_DD = 0xFF;
//	  u32 Tmp_HH = 0xFF, Tmp_MI = 0xFF, Tmp_SS = 0xFF;
  	struct rtc_time tm;
	 
	  if(((TimeCorrect->Time_Mon)<=12)&&((TimeCorrect->Time_Mday)<=31)&&
			((TimeCorrect->Time_Hour)<=23)&&((TimeCorrect->Time_Min)<=60)&&((TimeCorrect->Time_Sec)<=60))
		{
				RTC_Configuration();
		...
		{
       #if DEBUG_TEST	
       printf("\r\nWarning!!Server time config not right!\r\n"); //测试使用
			 #endif
    }
}
